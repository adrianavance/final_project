...
p5 <- program_participants %>%
  ...
  labs(title = "2017 National School Lunch Program",
...
p6 <- program_participants %>%
  ...
  labs(title = "2017 School Breakfast Program",
       x = "Children proportion") +
  theme_minimal()
...
c2 <- training %>%
  ...
  geom_point() + 
  geom_smooth() +
  labs(title = "2012 SNAP-authorized stores",
       x = "Insecurity value",
       y = "Stores/1,000 pop") +
  theme_minimal()
...
c3 <- training %>%
  ...
  geom_point() + 
  geom_smooth() +
  labs(title = "2010 Population with low access to store",
       x = "Insecurity value",
       y = "Population count") +
  theme_minimal()
...
c4 <- training %>%
  ...
  geom_point() + 
  geom_smooth() +
  labs(title = "2016 Grocery stores",
       x = "Insecurity value",
       y = "Store count") +
  theme_minimal()
...
Lastly, we seek to understand how certain policy changes may affect food insecurity levels in counties. We developed a simple microsimulation model with some of the more "important" variables in our data to simulate how a change in these policies could increase or decrease food insecurity in select counties.
...
Using a variable importance plot from our random forest model, we can see which predictors in our model are the most useful for predicting rates of food insecurity in counties in our dataset. Understanding which predictors are most important for predicting food insecurity can inform policy change. For example, "PCT_SBP12" and "PCT_SBP17" represent the percentage of School Breakfast Program participants at the state level. "PCT_WICWOMEN16" and "PCT_WICWOMEN16" represents the percentage of women participating in WIC in 2014 and 2016. Other important predictors are childhood poverty, rates of diabetes, and SNAP participation. Overall, it looks like county participation rates in state-level programs are more important predictors than county-level characteristics.

For simplicity, we will apply a calculator to one of the most important predictors, participation of women in WIC. Modeling increased participation in the WIC program can give us an idea of how a change to this policy may affect food insecurity rates in counties in the United States.
...
# Calculate the change in food insecurity
change_in_food_insecurity <-  predicted_food_insecurity - atlas_chr_snap$food_insecurity_chr

change_in_food_insecurity$.pred %>%
  summary()
...
```{r}
plus_10_pred <- cbind(predicted_food_insecurity, atlas_chr_snap$food_insecurity_chr, atlas_chr_snap)

dist_table <- plus_10_pred %>%
...
participation_increase <- dist_table |>
  ggplot(aes(pop_decile, mean_food_insecurity)) +
  geom_col() +
  ...
  labs(
    ...
    subtitle = "Increase in WIC Participation",
    x = "Familiy Income Decile",
    y = "Average Benefit"
  )

dist_table_original <- plus_10_pred %>%
  mutate(pop_decile = ntile(Population_Estimate_2018, n = 10)) %>%
  group_by(pop_decile) %>%
  summarize(
    min_pop = min(Population_Estimate_2018),
    max_pop = max(Population_Estimate_2018),
    mean_food_insecurity = mean(atlas_chr_snap$food_insecurity_chr)
  )

dist_table_original

participation_original <- dist_table_original |>
  ggplot(aes(pop_decile, mean_food_insecurity)) +
  geom_col() +
  scale_x_continuous(breaks = 1:10) +
  scale_y_continuous(
...
change_minus_10 <- food_insecurity_minus_10 - atlas_chr_snap$food_insecurity_chr
