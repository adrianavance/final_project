...
We ran anti-join commands to check for counties in each dataset that were not present in the other dataset, and removed two counties from 2024 and one county from 2023 as a result. We then merged the two CHR datasets. In the next section, we will merge the combined CHR dataset with the food atlas data and the SNAP policy data. The following output are tibbles listing the counties that do not match between datasets. These are counties that were either changed or removed between data collection, so they will be removed.
...
Compared to other programs, the WIC program has a relatively lower proportion of participants. All six density plots are not showing similar distribution patterns.

-   The proportion of WIC participants concentrate around 1.9% to 2.4%; children and infant proportions are higher, around 21% and 33%, while women participant proportions have multiple peaks, around 2%, 2.4%, 2.7% and 2.9%. - SNAP participant proportion in most counties is around 8% or 14%.
...
Lastly, we visualize this change in food insecurity with a line plot. While the change is incremental, it is clear that WIC participation has a relationship with food insecurity levels in counties, and the strength of this relationship varies across population levels of counties.
