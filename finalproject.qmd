...
In this project, we aim to predict the proportion of people in a county experiencing food insecurity using county-level data on sociodemographics, community characteristics, and food environment, as well as state-level data on Supplemental Nutrition Assistance Program (SNAP) policies. We will use lasso regression and random forest models to estimate food insecurity. We will then use the results of these analyses to create a microsimulation model to assess the impact that policy and program participation changes could have on food insecurity.
...
Next, we merged the combined food atlas/CHR data with the state-level SNAP data. We conducted a left join to map state level data onto each county. We then removed variables that directly indiciate food security/insecurity, other than our outcome variable from CHR.
