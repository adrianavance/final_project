...
In 2021, approximately 10% of American households experienced food insecurity - a lack of access to enough food for a healthy, active life - at some point during the year [@rabbitt]. The percentage of people experiencing food insecurity varies significantly between counties, ranging from 2% to 26% [@hake2023]. Understanding the key predictors of food insecurity at the county level could help us target interventions and resources where they're needed most and identify policies that are effective at reducing or preventing food insecurity.

In this project, we aim to predict the proportion of people in a county experiencing food insecurity using county-level data on sociodemographics, community characteristics, and food environment, as well as state-level data on Supplemental Nutrition Assistance Program (SNAP) policies. We will use lasso regression and random forest models to estimate food insecurity, the aim of which will be to use these past county characteristics to be able to predict food insecurity in the future. We will then use the results of these analyses to create a microsimulation model to assess the impact that policy and program participation changes could have on food insecurity.

## Data Sources

We compiled data from three different sources to identify the factors predicting rates of food insecurity at the county level in the United States:

-   **County Health Rankings.** The County Health Rankings (CHR) provide annual data for hundreds of health-related indicators at the county level. We pulled our outcome measure - the proportion of people in the county experiencing food insecurity in 2021 - from the most recent data release (CHR 2024). We pulled data from the previous data release (CHR 2023) for several predictor variables in our model. These variables include sociodemographic and economic measures like race/ethnicity, income inequality, high school graduation rates, unemployment, and cost burden of housing and childcare; percent living in a rural area; and other environmental and community factors like air pollution, traffic volume, and access to exercise opportunities. We used previous research on the factors associated with food security at the individual and state level to select the CHR variables to include in our dataset [@bartfeld2006][@hunt2019][@nicklett2023a].
-   **USDA Food Environment Atlas**. The USDA Food Environment Atlas contains 280 indicators related to the food environment for each county in the U.S., including data on food access/availability, poverty, participation in nutrition benefits and programs, and sociodemographics. It includes data for multiple time points from 2010-2017.
-   **SNAP Policy Datasets.** The USDA SNAP Policy Datasets contain detailed data on state-level SNAP policies, including broad-based categorical eligibility for SNAP benefits, recertification, waiver of face-to-face interviews for SNAP, eligibility restrictions on legal noncitizens' receipt of SNAP, and treatment of vehicles as part of the SNAP asset test. We pulled data from 2010-2020.
...
Alternatively, our data can be accessed in the data folder on our github repository.
...
The following code loads each of the datasets. 
...
Here, we selected predictor variables from the CHR 2023 dataset and selected the food insecurity outcome measure from the CHR 2024 dataset. We used the 2023 dataset for our predictors to ensure that all predictor variables predated our outcome variable. We removed the rows for national and state-level data and kept data for counties.  
...
#### Removing Inconsistent County Observations
We ran anti-join commands to check for counties in each dataset that were not present in the other dataset, and removed two counties from 2024 and one county from 2023 as a result. We then merged the two CHR datasets. In the next section, we will merge the combined CHR dataset with the food atlas data and the SNAP policy data.
The following output are tibbles listing the counties that do not match between datasets. These are counties that were either changed or removed between data collection, so they will  be removed.
...
Here, we filter out the counties.
```{r}
...
The following anti-joins show that there are no longer any county observations that do not match.
```{r}
anti_join(chr_2024_small, chr_2023_small, by = "fips")

anti_join(chr_2023_small, chr_2024_small, by = "fips")
```
Here, we join the final County Health Rankings dataset.
```{r}
chr_small <- 
...
The anti joins below show the counties in the food atlas dataset that are not represented in the CHR dataset, and vice versa. All of these counties were either changed or deleted in the time that these data were collected. In order to be able to merge our datasets, we will remove these counties from the data.
...
We removed the counties that are not present in both datasets, confirmed that the datasets now match, and then merged the food atlas and CHR data.
...
Here, we join the Food Atlas and County Health Rankings datasets
```{r}
atlas_chr <- full_join(food_atlas, chr_small, by = "fips") 

```

Next, we merged the combined food atlas/CHR data with the state-level SNAP data. We conducted a left join to map state level data onto each county. We then removed variables that directly indicate food security/insecurity, other than our outcome variable from CHR.
...
Our final dataset contains information about county and state-level characteristics that our research has shown may have an impact on food insecurity. We aim to create a model that can use these past characteristics to predict food insecurity levels in the future. As described above, our data spans multiple time periods and covers county characteristics related to both food access and socioeconomic status of counties. The characteristics in the data range from years 2010-2023, and will be used to predict the County Health Rankings food insecurity estimates from 2024.
...
Comparing to other programs, WIC program has relatively lower proportion of participants. All six density plots are not showing similar distribution patterns. - The proportion of WIC participants concentrate around 1.9% to 2.4%; children and infant proportions are higher, around 21% and 33%, while women participant proportions have multiple peaks, around 2%, 2.4%, 2.7% and 2.9%. - SNAP participant proportion in most counties is around 8% or 14%. - The participation children proportions among children for both school lunch and school breakfast programs are much higher than the previous ones. Density of proportions in national school lunch program has three peaks: 50%, 64%, and 71%. The majority of the counties include more than half of their children within the lunch program. - The participation proportion distribution in breakfast is relatively lower, and only with two peaks: 24% and 35%.
...
Here, we perform feature selection based on the correlation coefficients between the numeric variables in our data and our target variable, non-return rate. We first filter out variables that don't have a correlation with non return rate. Since most factors have both a population and proportion variables, we only keep the proportion observations due to the variance in population. We also keep the number of blocks in track, and land area as a feature.

Here, we perform feature selection before our model specification to narrow down the number of predictors we will be using. Firstly, we measure the correlation between our variable of interest--food insecurity-- and our other variables. After, we filter out any variables that have a correlation of 0 with our food insecurity 

By doing this, we narrow down the number of predictors into about 350 variables instead of 604 variables in the original training data.
...
In our random forest model, we again assign ID roles to identification variables, eliminate non-zero and near-zero variance predictors, normalize predictors, and group together infrequent values into an "other" category. As for the hyperparameters, we increased the number of trees and mtrys which resulted in incremental improvements in our error metric.
...
The following output is a summary of the predicted values of food insecurity with a 20% increase in womens' participation in the WIC program. 
...
```{r}
change_plus_10$food_insecurity_original %>%
  summary()

```
In comparison with the summary of food insecurity at original levels, we can see that an increase in WIC participation may be associated with a slight increase in food insecurity, as average food insecurity is slightly higher with the participation change.
...
The above distribution table demonstrates how average food insecurity is distributed across 10 different levels of population in counties; this table will be used later to create a visualization comparing changes in participation.

### Modeling a Decrease in WIC Participation
The following output is a summary of food insecurity when womens' participation in WIC decreases by 20%. As we can see, average food insecurity decreases in comparison to original levels of participation, from approximately 0.1146 to 0.1137.
...
dist_table_decrease
```
Once again, this distribution table shows average food insecurity across population deciles with a decrease in WIC participation. This will be used to visualize the difference in participation below. 
...
### Conclusion
In our project we set out to create a predictive model that reliably predicts food insecurity levels in counties based on sociodemographic and policy characteristics. While we were unable to actually apply the model to implementation data, it seems that it performs reliably on unseen data. 

This model could be used to predict food insecurity levels in response to policy or environmental changes, such as participation in certain government programs. We modeled a change in WIC participation using one of our predictive models, and found that we could predict how food insecurity may change in response to changes in behavior. 

While this can be very useful in policy applications, the simulation model was quite simple-- it is clear that policy expertise is needed to understand exactly how program participation would change in order to increase the accuracy of predictions.

Food insecurity is a persistent problem in communities across the United States, and county-level policy changes may be helpful in mitigating its negative effects. From our variable importance plot and correlation tests, we found that many important predictors of food insecurity are policies that occur at the state level. However, through our predictive models and simulation, we found that other county-level characterstics such as poverty levels and program participation can change how these state-level policies interact with food insecurity levels. 

In conclusion, our predictive models and simulation provide a helpful view into how county-level characteristics and food policies can affect food insecurity. Our simulation, informed by policy and behavioral expertise can be helpful in predicting how food insecurity may change due to policy or environmental changes. 
