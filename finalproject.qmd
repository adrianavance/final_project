...
The density distribution of the program participation proportions do not show the similar trends as we expected before, so we will do the correlation test to determine wether these variables will be included as predictors.
...
correlation_df <- as.data.frame(correlation) %>%
  ...
  na.omit() %>% # filter out variables that don't have correlation with y
...
```{r}
sub_folds <- vfold_cv(data = sub_training, v = 10, repeats = 10)
